...
my $usage = "Usage: $0 GenBank-INFILE(s) ITR5-OUTFILE ITR3-OUTFILE";
my $out3file = pop or die $usage;
my $out5file = pop or die $usage;
my @infiles = @ARGV;
@infiles > 0 or die $usage;
...
# output
		...
		$out3->write_seq($ITR3_seqobj); # use the revcom seq as the adapter sequence
	}
	$in->close();
}

...
# prepare get vector seq and annotation cmd
	{
		...
		my $seq_out = $design->get_sample_vec_seqfile($sample);
		my $anno_out = $design->get_sample_vec_annofile($sample);
...
# prepare build vector db and map cmd
  {
		...
		my $in = $design->get_sample_vec_seqfile($sample);
    ...
    my $cmd;
      ...
      $cmd .= "\nbowtie2 -x $WORK_DIR/$dbname -1 $WORK_DIR/$in1 -2 $WORK_DIR/$in2 $align_opts -p $NUM_PROC | samtools view -b -o $WORK_DIR/$out -";
		}
		else {
